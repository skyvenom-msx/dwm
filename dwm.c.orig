...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		if (i < LENGTH(tags)) {
			...
			click = ClkLtSymbol;
		else if (ev->x > selmon->ww - dwmblocksw) {
			x = selmon->ww - dwmblocksw;
			click = ClkStatusText;
			dwmblockssig = 0;
	...
	if (m == selmon || 1) { /* status is only drawn on selected monitor */
		...
		for (text = s = stext; *s; s++) {
			if ((unsigned char)(*s) < ' ') {
				...
				*s = '\0';
				tw = TEXTW(text) - lrpad;
				drw_text(drw, m->ww - dwmblocksw + x, 0, tw, bh, 0, text, 0);
				x += tw;
				*s = ch;
				text = s + 1;
			}
...
updatestatus(void)
{
	...
	if (!gettextprop(root, XA_WM_NAME, stext, sizeof(stext))) {
		...
		dwmblocksw = TEXTW(stext) - lrpad + 2;
	} else {
		char *text, *s, ch;
		...
		for (text = s = stext; *s; s++) {
			if ((unsigned char)(*s) < ' ') {
				ch = *s;
				*s = '\0';
				dwmblocksw += TEXTW(text) - lrpad;
				*s = ch;
				text = s + 1;
			}
		}
		dwmblocksw += TEXTW(text) - lrpad + 2;
